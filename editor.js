...
function floatPropertiesPanel(requestTab = null) {
    ...
    ipcRenderer.invoke('open-panel', 'properties', {
        ...
        allFieldNames:  buildAllFieldNamesList(),
    });
}

/**
 * Flat list of all fields in the document, lightweight enough to serialize
 * for IPC. Used by the JS Snippet Library's field-picker dropdowns and
 * the canvas-pick button so users don't have to type field names by hand.
 */
function buildAllFieldNamesList() {
    return fields
        .filter(f => f && f.name)
        .map(f => ({ name: f.name, type: f.type, page: f.page }));
}
...
ipcRenderer.on('from-panel', (event, { action, payload }) => {
    switch (action) {
        ...
        // ── Properties panel: canvas pick for the JS Snippet Library ─────────
        case 'js-lib-pick-start': {
            // Activate pick mode for a JS Library param. The paramId tells the
            // panel which input to fill; multi=true means stay in pick mode and
            // append each click (for fields like the comma-separated "checkboxes
            // to uncheck" list).
            if (isPickModeActive()) disablePickMode();
            const paramId = payload?.paramId || null;
            const isMulti = !!payload?.multi;
            const preSeed = payload?.names || [];
            pickMode = {
                btnId: null,
                pickedNames: new Set(preSeed),
                onDisable: () => {
                    ipcRenderer.send('editor-to-panel', 'properties', {
                        jsLibPickStopped: true, paramId,
                    });
                },
                onPick: (field) => {
                    ipcRenderer.send('editor-to-panel', 'properties', {
                        jsLibPickResult: { paramId, fieldName: field.name },
                    });
                    // For single-pick params, deactivate after the first click.
                    if (!isMulti) disablePickMode();
                },
            };
            if (canvas) canvas.classList.add('canvas-pick-mode');
            renderCanvas();
