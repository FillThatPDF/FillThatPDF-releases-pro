...
ipcRenderer.on('panel-update', (evt, { type, data }) => {
    if (type === 'global-styles') {
        ...
        // Keep the JS Snippet Library's field-name autocomplete fresh.
        // The list arrives on every properties refresh; missing payloads
        // (e.g. pick events) leave the cached list intact.
        if (data?.allFieldNames && typeof window.jsLibSetAllFields === 'function') {
            window.jsLibSetAllFields(data.allFieldNames);
        }
        if (data?.jsLibPickResult && typeof window.jsLibOnPickResult === 'function') {
            // Editor reports a field was clicked while a JS Library pick is active
            window.jsLibOnPickResult(data.jsLibPickResult);
            return;
        }
        if (data?.jsLibPickStopped && typeof window.jsLibOnPickStopped === 'function') {
            window.jsLibOnPickStopped(data.jsLibPickStopped === true ? null : data.paramId);
            return;
        }
        if (data?.pickResult) {
...
function initProperties(data) {
    ...
    // Seed the JS Snippet Library's field-name autocomplete with the current
    // document's fields so dropdowns and the canvas-pick button can use them.
    if (data?.allFieldNames && typeof window.jsLibSetAllFields === 'function') {
        window.jsLibSetAllFields(data.allFieldNames);
    }
}
...
function propPopulate(field, globalSettings) {
    ...
    // Keep the six Position inputs in sync. propSendUpdate only reads
    // Left/Top/Right/Bottom for the rect — without this, editing Width or
    // Height has no effect because the corresponding Right/Bottom values
    // stay at their old numbers.
    propWirePositionSync();
...
/**
 * Cross-input synchronisation for the six Position fields.
 *
 *   Left   ↔  Right − Width
 *   Top    ↔  Bottom − Height
 *   Width  =  Right − Left
 *   Height =  Bottom − Top
 *
 * Editing any one field cascades into the related fields so the rect stays
 * geometrically consistent. The actual save (in propSendUpdate) reads
 * Left / Top / Right / Bottom — and those will be correct because Width and
 * Height edits propagate to Right and Bottom here.
 *
 * Moving Left or Top keeps Width / Height constant (it's a move). Changing
 * Right or Bottom keeps Left / Top constant (it's a resize from the right /
 * bottom). Changing Width or Height keeps Left / Top constant.
 *
 * Safe to call repeatedly: a one-shot flag on the panel root guards against
 * double-wiring when the user switches between fields.
 */
function propWirePositionSync() {
    const root = document.getElementById('propertiesPanelFloat');
    if (!root || root._positionSyncWired) return;
    root._positionSyncWired = true;
    ...
    function syncFromLeft() {
        const left  = num('modalPropLeft');
        const width = num('modalPropWidth');
        if (Number.isFinite(left) && Number.isFinite(width)) {
            setNum('modalPropRight', left + width);
        }
    }
    function syncFromTop() {
        const top    = num('modalPropTop');
        const height = num('modalPropHeight');
        if (Number.isFinite(top) && Number.isFinite(height)) {
            setNum('modalPropBottom', top + height);
        }
    }
    ...
    function syncFromRight() {
        const left  = num('modalPropLeft');
        const right = num('modalPropRight');
        if (Number.isFinite(left) && Number.isFinite(right)) {
            setNum('modalPropWidth', right - left);
        }
    }
    function syncFromBottom() {
        const top    = num('modalPropTop');
        const bottom = num('modalPropBottom');
        if (Number.isFinite(top) && Number.isFinite(bottom)) {
            setNum('modalPropHeight', bottom - top);
        }
    }

    $('modalPropLeft')  ?.addEventListener('input', syncFromLeft);
    $('modalPropTop')   ?.addEventListener('input', syncFromTop);
    $('modalPropWidth') ?.addEventListener('input', syncFromWidth);
    $('modalPropHeight')?.addEventListener('input', syncFromHeight);
    $('modalPropRight') ?.addEventListener('input', syncFromRight);
    $('modalPropBottom')?.addEventListener('input', syncFromBottom);
}
...
function jsLibOnPresetChange() {
    ...
    if (typeof jsLibStopAllPicks === 'function') jsLibStopAllPicks();
    ...
    if (paramsEl) {
        if (preset.params.length > 0) {
            ...
            // jsLibRenderParam (defined in panel_properties.js) returns the
            // appropriate widget for each param type — including 'field'
            // typeahead with 🎯 button, 'fields' chip-list with multi-pick,
            // and 'select' dropdowns.
            paramsEl.innerHTML = preset.params.map(jsLibRenderParam).join('');
            if (typeof jsLibWireParamControls === 'function') {
