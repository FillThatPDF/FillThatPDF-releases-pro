...
def get_text_value(field_name: str, field=None, settings: FillSettings = None, is_calc_source: bool = False, calc_source_scale: str = 'small', max_len: int = 0) -> str:
    ...
    # Field names containing the word "number"/"numbers" should have an
    # AFNumber_Format (or similar) action set on them in a properly built form.
    # If no format JS was detected above, do NOT inject a random integer — that
    # masks a missing format action. Emit Sample/Example so the bug is visible.
    # Word-boundary match avoids hits inside unrelated words (lumber, encumbered).
    if re.search(r'\bnumbers?\b', name_lower):
        return random.choice(['Sample', 'Example'])
    ...
    # No format JS detected on this field. For format-prone field types
    # (phone, zip, date) do NOT hand-format the value from the field name —
    # that would mask a missing /AA/F format action and produce a false-positive
    # test. Emit plain "Sample"/"Example" so an unformatted field is visually
    # obvious. Fields with a real format action are handled in the branch above.
    if 'phone' in name_lower or 'tel' in name_lower:
        ...
        return random.choice(['Sample', 'Example'])
    elif 'state' in name_lower: return random.choice(SAMPLE_DATA['states'])
...
if __name__ == '__main__':
    ...
    # main.js (run-test-fill) passes --assets-dir pointing at the bundled
    # test-fill icons so image-upload pushbutton fields can be filled with
    # sample logos. This script doesn't yet consume that data, but accepting
    # the arg here prevents argparse from exiting with code 2.
    parser.add_argument("--assets-dir", help="Directory of test-fill assets (logos for image-upload pushbutton fields)")
