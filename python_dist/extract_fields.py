...
def extract_fields_and_images(pdf_path: str, output_dir: str = None) -> dict:
    ...
    # Scale for rendering (150 DPI / 72 DPI standard)
    zoom = 150 / 72
            ...
            for annot in page_pike.Annots:
                ...
                if actions:
                    ...
                    if 'Calculate' in actions:
                        ...
                        if calc_info:
                            # Carry the field's readOnly flag into the calc object
                            # so the editor can display it accurately in the
                            # Calculate tab without relying on the HTML default.
                            calc_info['readOnly'] = field_info.get('readOnly', False)
...
def _parse_calculate_js(js_str: str, field_name: str = None) -> dict:
    ...
    # Match var-based CONSTANT_MULTIPLY pattern (generated by older versions):
    # event.value = NUMBER * (v0);  or  event.value = NUMBER * (v0 * v1);
    # where field_refs were extracted from getField() calls in var declarations
    const_var_match = re.search(
        r'event\.value\s*=\s*([\d.]+)\s*\*\s*\(',
        js_str
    )
    if const_var_match and field_refs:
        # Verify the expression after the constant only contains var refs and *
        ev_match = re.search(r'event\.value\s*=\s*[\d.]+\s*\*\s*\(([^)]+)\)', js_str)
        if ev_match:
            inner = ev_match.group(1).strip()
            # Check it's only v0, v1... multiplied together (no other operators)
            if re.match(r'^v\d+(?:\s*\*\s*v\d+)*$', inner):
                return {
                    "type": "CONSTANT_MULTIPLY",
                    "sources": field_refs,
                    "constant": float(const_var_match.group(1))
                }
