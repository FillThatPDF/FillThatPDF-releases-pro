...
class PageAnalyzer:
    ...
    def _build_visual_grid(self, page_num: int, tables, edges,
        ...
        for table in tables:
            try:
                if not (hasattr(table, 'cells') and table.cells):
                    continue
                table_cells = []
                for cell in table.cells:
                    if cell and len(cell) == 4:
                        ...
                        if cx1 > cx0 and cy1 > cy0:
                            table_cells.append((cx0, cy0, cx1, cy1))

                inferred = self._infer_checkerboard_cells(table_cells, h_edges)
                cells.extend(table_cells)
                cells.extend(inferred)
            except Exception:
                continue

        return cells

    def _infer_checkerboard_cells(self, table_cells, h_edges,
                                   x_tol: float = 1.5, y_tol: float = 1.5):
        """
        Recover cells in checkerboard tables — rows where pdfplumber missed
        the leftmost/rightmost columns because vertical edges only existed
        on alternating (e.g., zebra-shaded) rows.

        A cell is added only if both its top and bottom y-coordinates have
        a horizontal edge spanning the full column x-range, so we never
        invent cells without geometric support.
        """
        if not table_cells or not h_edges:
            return []

        def col_key(cx0, cx1):
            return (round(cx0 / x_tol) * x_tol, round(cx1 / x_tol) * x_tol)

        col_ranges = {}
        for c in table_cells:
            k = col_key(c[0], c[2])
            if k not in col_ranges:
                col_ranges[k] = (c[0], c[2])
        ...
        for c in table_cells:
            by_row[round(c[1], 1)].append(c)

        h_by_y = defaultdict(list)
        for e in h_edges:
            h_by_y[round(float(e.get('top', e.get('y0', 0))), 1)].append(
                (float(e['x0']), float(e['x1']))
            )

        def h_span(y, cx0, cx1):
            for ye, segs in h_by_y.items():
                if abs(ye - y) > y_tol:
                    continue
                for hx0, hx1 in segs:
                    if hx0 <= cx0 + x_tol and hx1 >= cx1 - x_tol:
                        return True
            return False

        existing = {(round(c[0], 1), round(c[1], 1), round(c[2], 1), round(c[3], 1))
                    for c in table_cells}

        new_cells = []
        for y0, row in by_row.items():
            present = set(col_key(c[0], c[2]) for c in row)
            if len(present) >= len(col_ranges):
                continue
            y1 = max(c[3] for c in row)
            for k, (cx0, cx1) in col_ranges.items():
                if k in present:
                    continue
                if not (h_span(y0, cx0, cx1) and h_span(y1, cx0, cx1)):
                    continue
                key = (round(cx0, 1), round(y0, 1), round(cx1, 1), round(y1, 1))
                if key in existing:
                    continue
                new_cells.append((cx0, y0, cx1, y1))
                existing.add(key)
        return new_cells
