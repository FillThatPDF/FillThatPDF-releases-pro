...
def _is_strong_color(fill) -> bool:
    """True when *fill* is a non-white colour (CMYK / RGB / greyscale)."""
...
def _is_colored_header_cell(pm, cx0, cy0, cx1, cy1) -> bool:
    """
    True when a cell coincides with a non-white filled rect that contains
    text — i.e., the cell IS the colored header label, not a data cell.

    Detectors must skip these so they don't manufacture phantom entry
    fields inside the header (which would later be removed by
    ColoredHeaderFilter and crowd out legitimate first-body-row cells in
    the resolver).
    """
    cell_h = cy1 - cy0
    cell_w = cx1 - cx0
    if cell_h > 35 or cell_h < 5 or cell_w < 30:
        return False
    for r in pm.rects:
        if not _is_strong_color(r.get('non_stroking_color')):
            continue
        rx0 = float(r.get('x0', 0))
        ry0 = float(r.get('top', 0))
        rx1 = float(r.get('x1', 0))
        ry1 = float(r.get('bottom', 0))
        if (abs(rx0 - cx0) > 3 or abs(rx1 - cx1) > 3
                or abs(ry0 - cy0) > 3 or abs(ry1 - cy1) > 3):
            continue
        txt = pm.get_text_in_bbox((rx0 + 1, ry0 + 1, rx1 - 1, ry1 - 1), 0.3)
...
class TableCellDetector(BaseDetector):
    ...
    def _grid_text_path(self, pm, cells, prefilled_cols, v_lines, global_headers_map, column_classifications=None) -> List[FieldCandidate]:
        ...
        for c in cells:
            ...
            # Skip cells that ARE the colored header (don't manufacture
            # phantom entry fields inside header labels).
            if _is_colored_header_cell(pm, cx0, cy0, cx1, cy1):
                continue
