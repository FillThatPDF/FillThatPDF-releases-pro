...
class PDFWriter:
    ...
    def _create_text_widget(self, pdf, name, tx0, ty0, tx1, ty1,
        ...
        # Text field flag bits (/Ff for Tx fields):
        #   bit 13 (4096)       — Multiline (allow text wrapping in tall fields)
        #   bit 24 (0x800000)   — DoNotScroll (don't scroll text that exceeds the
        #                         visible area)
        #
        # We default to DoNotScroll ON. That matches the "Scroll long text"
        # checkbox in Field Properties → Options being UNCHECKED, which is how
        # the modal renders by default. Previously /Ff was 0 (or 4096 for tall
        # fields) — scrolling was always enabled in the generated PDF even
        # though the modal said it wasn't, which made the two views disagree.
        # User can still enable scrolling on a per-field basis by checking
        # the modal box; modify_fields.py clears bit 24 when scrollText=true.
