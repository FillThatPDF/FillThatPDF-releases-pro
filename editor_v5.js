...
// Helper: record a field move / resize in both name-based and objgen-based
// change maps. objgen (PDF object number) is stable across saves — unlike
// sequential indices which shift when other fields are deleted.
//
// IMPORTANT: A field can land in BOTH `moved` and `resized` if the user moves
// it after resizing (or vice-versa). Each entry stores a rect SNAPSHOT taken
// at record time, so the snapshots diverge. modify_fields.py merges the two
// dicts with `{**moved, **resized}`, which means resized always wins on key
// collision — and if the latest action was a move, the stale resize-time
// rect gets applied instead, silently losing the move.
//
// Fix: whenever we record one side, propagate the new rect to the OTHER side
// if it's already tracking this field. Both snapshots stay in sync, the
// merge order stops mattering, and Python applies the user's actual final
// rect either way.
function _syncOtherRect(otherMap, name, rect) {
    if (otherMap && otherMap[name]) {
        otherMap[name] = { ...otherMap[name], rect: [...rect] };
    }
}
function recordFieldMove(field, rect) {
    const r = [...rect];
    changes.moved[field.name] = { rect: r, page: field.page };
    _syncOtherRect(changes.resized, field.name, r);
    if (field.objgen) {
        const og = field.objgen.join(',');
        if (!changes.moved_by_objgen) changes.moved_by_objgen = {};
        changes.moved_by_objgen[og] = { rect: [...r], page: field.page, name: field.name };
        _syncOtherRect(changes.resized_by_objgen, og, r);
    }
}
function recordFieldResize(field, rect) {
    const r = [...rect];
    changes.resized[field.name] = { rect: r, page: field.page };
    _syncOtherRect(changes.moved, field.name, r);
    if (field.objgen) {
        const og = field.objgen.join(',');
        if (!changes.resized_by_objgen) changes.resized_by_objgen = {};
        changes.resized_by_objgen[og] = { rect: [...r], page: field.page, name: field.name };
        _syncOtherRect(changes.moved_by_objgen, og, r);
    }
}
...
function setupScrollViewMarquee(container) {
    ...
    container.addEventListener('contextmenu', (e) => {
        ...
        e.preventDefault();
        // No field under the cursor → clear any stale right-click reference
        // so a subsequent context-menu action falls back to selectedFields[0].
...
function svBuildOverlay(field, wrap, iw, ih) {
    ...
    ov.addEventListener('contextmenu', (e) => {
        ...
        e.stopPropagation();
        const f = liveField();
        // Track the right-clicked field so context-menu actions like
        // "Same Height" use IT as the reference (sizing all others UP to
        // match) rather than selectedFields[0] (which could be any of the
        // smaller fields the user shift-selected earlier).
        contextMenuField = f || null;
...
async function handleSave() {
    ...
    // Diagnostic: log the changes being sent so we can see exactly what reaches
    // the Python script. Especially helpful for move-persistence issues.
    try {
        const summary = {
            moved:   Object.keys(changes.moved   || {}).length,
            resized: Object.keys(changes.resized || {}).length,
            styled:  Object.keys(changes.styled  || {}).length,
            renamed: Object.keys(changes.renamed || {}).length,
            deleted: (changes.deleted || []).length,
        };
        console.log('[Save] changes summary:', summary);
        if (summary.moved > 0) {
            console.log('[Save] moves:', JSON.stringify(changes.moved, null, 2));
        }
        if (changes.moved_by_objgen) {
            console.log('[Save] moves_by_objgen:', JSON.stringify(changes.moved_by_objgen, null, 2));
        }
    } catch (_) {}
...
function hideContextMenu() {
    ...
    // contextMenuField stays set until handleContextMenuAction consumes it.
}

// Tracks the field the user right-clicked on. Used as the *reference* for
// alignment / "Same Size" actions invoked from the context menu, so that
// right-clicking the larger field and choosing "Same Height" sizes the
// others UP to match it (instead of squashing the right-clicked field down
// to selectedFields[0]'s height).
...
function handleContextMenu(e) {
    ...
    contextMenuField = clickedField || null;
...
function handleContextMenuAction(action) {
    hideContextMenu();
    // Snapshot the right-clicked field for this action and clear the slot so
    // any subsequent toolbar-button invocation of matchSize won't see a stale
    // reference from an earlier menu interaction.
    const ctxField = contextMenuField;
    contextMenuField = null;
    ...
    switch (action) {
        ...
        case 'distribute-v':
            ...
            break;
        case 'match-width':
            matchSize('width', ctxField);
            break;
        case 'match-height':
            matchSize('height', ctxField);
            break;
        case 'match-both':
            matchSize('both', ctxField);
            break;
        case 'duplicate':
            duplicateSelectedFields();
            break;
...
function matchSize(dimension, referenceField) {
    if (selectedFields.length < 2) return;

    // Prefer the explicitly-passed reference (the right-clicked field from
    // the context menu) so right-clicking the larger field and choosing
    // "Same Height" sizes the others UP to match it. Fall back to
    // selectedFields[0] for toolbar-button invocations where no reference
    // is supplied.
    const ref = (referenceField && selectedFields.includes(referenceField))
        ? referenceField
        : selectedFields[0];
...
// ── Promote field-name params to first-class types (see panel_properties.js for prose) ──
(function applyJsLibParamTypes() {
    ...
    const NOT_FIELD = new Set(['form-open-embedded:name', 'nav-named-dest:dest']);
    ...
    for (const preset of JS_PRESETS) {
        ...
        for (const param of preset.params) {
            ...
            if (param.type === 'text' || !param.type) {
                if (FIELD_IDS.has(param.id))      param.type = 'field';
                else if (FIELDS_IDS.has(param.id)) param.type = 'fields';
            }
...
function jsLibCurrentFieldNames() {
    return (typeof fields !== 'undefined' && Array.isArray(fields))
        ? fields.filter(f => f && f.name).map(f => ({ name: f.name, type: f.type }))
        : [];
}
...
function jsLibRefreshFieldDatalist() {
    ...
    const list = jsLibCurrentFieldNames();
    dl.innerHTML = list.map(f => `<option value="${jsLibEscapeAttr(f.name)}">${jsLibEscapeAttr(f.type || '')}</option>`).join('');
}
...
function jsLibWireParamControls() {
...
function jsLibAddFieldChipFromInput(paramId) {
...
function jsLibRemoveFieldChip(xEl) {
...
// Canvas-pick coordination (uses the editor's existing pickMode directly — no IPC)
function jsLibTogglePick(paramId, isMulti, btn) {
    const active = _jsLibActivePicks.has(paramId);
    if (active) {
        if (typeof disablePickMode === 'function') disablePickMode();
    ...
    jsLibStopAllPicks();
    if (typeof isPickModeActive === 'function' && isPickModeActive() && typeof disablePickMode === 'function') {
        disablePickMode();
    }
    ...
    pickMode = {
        ...
        onPick: (field) => {
            jsLibOnPickedField(paramId, field.name, isMulti);
        },
    };
    if (typeof canvas !== 'undefined' && canvas) canvas.classList.add('canvas-pick-mode');
    if (typeof renderCanvas === 'function') renderCanvas();
}

function jsLibOnPickedField(paramId, fieldName, isMulti) {
    const info = _jsLibActivePicks.get(paramId);
    if (!info) return;
    if (isMulti) {
        const cur = jsLibGetFieldsValue(paramId);
...
function jsLibStopAllPicks() {
    ...
    for (const [, info] of _jsLibActivePicks) {
        if (info.btn) {
            info.btn.classList.remove('active');
            info.btn.textContent = '🎯';
        }
...
function jsLibOnPresetChange() {
    ...
    // Stop any active canvas-pick from a previous preset
    jsLibStopAllPicks();
